...
class Settings(BaseModel):
    ...
    trade_history_file: str = "data/history/trade-history.jsonl"
    strategy_state_file: str = "data/history/strategy-state.json"
    ...
    @property
    def trade_history_path(self) -> str:
        return self.trade_history_file or str(Path(self.persistent_data_dir) / "history" / "trade-history.jsonl")

    @property
    def strategy_state_path(self) -> str:
        return self.strategy_state_file or str(Path(self.persistent_data_dir) / "history" / "strategy-state.json")
...
def get_settings() -> Settings:
    ...
    return Settings(
        ...
        trade_history_file=os.getenv("TRADE_HISTORY_FILE", str(Path(persistent_data_dir) / "history" / "trade-history.jsonl")),
        strategy_state_file=os.getenv("STRATEGY_STATE_FILE", str(Path(persistent_data_dir) / "history" / "strategy-state.json")),
