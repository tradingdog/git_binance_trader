...
## v0.1.13 - 2026-04-08
- **修复模拟仓位价格严重失真问题**：
  - 根因：`_open_mock_position` 对所有交易对使用同一个随机价格区间 8-160 USDT 和数量 0.4-1.8，导致 BTC 价格显示为 ~28 USDT（真实约 71,639 USDT）、数量显示为 1.17 BTC（10,000 USDT 账户无法持有）。
  - 修复：为每个交易对配置独立的真实参考价格区间（BTC 68,000-74,000 / ETH 3,200-3,600 / SOL 140-160 / DOGE 0.15-0.25 / AI 0.5-1.5 / WLD 1.5-3.0 USDT），并对应调整数量范围，使单笔保证金约 200-600 USDT（≤账户净值的 6%）。
  - 修复 `_append_market_timeseries`：行情时序 OHLCV 数据同样使用了错误的 10-120 基础价格，改为按交易对独立的真实价格区间，成交量也按币种实际量级区分（BTC 50-300 / DOGE 500,000-5,000,000 基础币）。
  - 10/10 测试通过；示例输出：BTC 71,836 @0.003125 BTC 保证金 224 USDT，DOGE 0.239 @3,347 DOGE 保证金 267 USDT。

## v0.1.12 - 2026-04-08
- **修复引擎启动即halt导致程序无动静的严重bug**：
  - 根因：`_close_mock_position` 中单笔亏损红线用 `margin * 1%` 计算上限，而保证金最低只有 2-3 USDT，正常价格波动（<1%）产生的亏损就必然超限，每次平仓必触发 halt。
  - 修复：将阈值改为 `equity * max_trade_loss_pct / 100`（账户净值的1% = ~100 USDT），符合规范"单笔亏损 <= 1% 账户"。
  - 附加修复：`engine.start()` 每次启动时强制将 `status` 重置为 `running`，避免持久化或测试残留的 halted 状态在部署重启后滞留。
  - 10/10 测试通过。
