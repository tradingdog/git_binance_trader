...
## v0.1.15 - 2026-04-08
- 在 v0.1.14 市场池对齐基础上完成线上稳定性收口：
  - 修复启动阶段与市场池刷新的阻塞风险，主循环增加异常保护，避免后台任务意外停止。
  - 增加外网失败回退市场池，保证交易对覆盖不中断。
  - 交易对符号增加标准化过滤，仅保留 `A-Z0-9`，去除异常符号。
  - Fly 运行规格固定为 512MB 内存，解决实例反复 OOM 重启。
  - 线上验证：`/health` 正常，`/api/ai/governance` 持续返回市场时序与交易数据。

## v0.1.14 - 2026-04-08
- **统一 AI 版与人类版市场池规则（核心修复）**：
  - 新增 `market_universe.py`，AI 版不再使用硬编码少量交易对，改为实时拉取 Binance 现货/永续/Alpha 市场池。
  - 与人类版对齐：
    - 现货/永续仅纳入 `TRADING` + `USDT` 交易对。
    - 剔除前20大市值交易对与稳定币基资产。
    - Alpha 仅纳入 24h 涨跌幅 `>= +10%` 或 `<= -10%` 的币，并执行流动性、24h成交额、24h成交笔数、30日中位日成交额过滤。
  - 引擎开仓逻辑改为基于统一市场池选币，仓位规模按净值 2%-6% 动态分配，修复“AI版只交易几个币”的结构性问题。
  - 行情时序（OHLCV）改为基于统一市场池动态生成，不再使用固定少量交易对。
  - 性能保护：改用轻量价格端点、Alpha 扫描上限 40，市场池刷新间隔可配置（默认 60 ticks），避免主循环阻塞与 OOM。
  - 稳定性修复：市场池刷新改为后台线程执行，且主循环首轮先 `sleep` 后运行，避免 FastAPI 启动阶段被同步IO阻塞导致实例启动超时/内存峰值异常。
  - 可靠性修复：主循环增加异常保护与审计记录，避免单次市场接口异常导致引擎后台任务中止。
  - 容灾回退：当外网受限导致 Binance 拉取失败时，自动启用“广覆盖模拟市场池”（大量 spot/perpetual + alpha），避免系统退化为少量交易对或无交易状态。
  - 符号规范化：新增交易对正则过滤（仅保留 `A-Z0-9`），剔除异常字符符号，避免看板出现非标准交易对名。
  - 部署稳定性：Fly 机器内存提升并固化为 512MB，解决线上进程被 OOM 周期性重启问题。
  - 新增配置：`market_universe_limit`（默认 300）、`market_universe_refresh_ticks`（默认 60）。
  - 测试结果：`python -m pytest -q` 通过（10/10）。
